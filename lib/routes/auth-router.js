...
module.exports = (passport) => {
    ...
    router.post('/login', authRequisites.RequiresNotLogin ,passport.authenticate('local', {failureRedirect:'/auth/login'}), (req, res) => {
        res.redirect('/');
    });
    ...
    router.post('/register',
        ...
        bodyValidator(['username', 'A username is required']).not().isEmpty(),
        bodyValidator(['password', 'The password must be alphanumeric and contain at least 8 characters']).trim().isAlphanumeric().isLength({min:8}),
        bodyValidator(['email', 'The email must be valid']).trim().isEmail(),
        (req, res) => {
