 ...
 */
function AddRecordToDB(moteId, timestamp, light, temperature, ledLevel){
    return knex('records').insert({mote_id:moteId, timestamp: timestamp, light: light, temperature: temperature, ledlevel: ledLevel});
}
...
/**
 * Returns a paginated version of the records, page represents the page and pageSize the number
 * of suggestions per page.
 * 
 * @param {any} [page=0] defaults to 0
 * @param {any} [pageSize=50] defaults to 20
 * @returns {Promise<Array<Object>>}
 */
function GetRecordsPage(page = 0, pageSize = 50){
    return knex('records').select('*').orderBy('id', 'desc').limit(pageSize).offset(page * pageSize);
}
...
function GetDistinctMotes(){
    return knex('records').distinct('mote_id').orderBy('mote_id', 'asc');
}
//-------------------------------------------
//                Suggestions
//-------------------------------------------
...
/**
 * Returns a paginated version of the suggestions, page represents the page and pageSize the number
 * of suggestions per page.
 * 
 * @param {any} [page=0] defaults to 0
 * @param {any} [pageSize=20] defaults to 20
 * @returns {Promise<Array<Object>>}
 */
function GetSuggestionsPage(page = 0, pageSize = 20){
    return knex('suggestions')
        .orderBy('suggestions.id', 'desc')
        .select(['suggestions.id', 'suggestions.suggestion', 'suggestions.rating', 'users.username'])
        .limit(pageSize)
        .offset(page * pageSize)
        .innerJoin('users', 'users.id', 'suggestions.userid');
}
...
module.exports = {
    ...
    GetDistinctMotes: GetDistinctMotes,
    ...
    GetSuggestionsPage: GetSuggestionsPage,
...
async function seedFakeRecords(numberToGen){
    ...
    for (let i = 0; i < numberToGen; i++) {
        let moteId = faker.datatype.number(2);
        ...
        await knex('records').insert({
            mote_id:moteId, timestamp:timestamp, light:light, temperature:temperature, ledlevel:ledLevel
        });
    }
}
